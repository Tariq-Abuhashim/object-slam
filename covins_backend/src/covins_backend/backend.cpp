...
CovinsBackend::CovinsBackend() {
    ...
    //+++++ Parameters +++++
    covins_params::ShowParamsComm();    // covins_comm   /src/covins_base/config_comm.cpp
    covins_params::ShowParamsBackend(); // covins_backend/src/covins_base/config_backend.cpp
...
auto CovinsBackend::ConnectSocket()->void {
    ...
    hints.ai_family = AF_UNSPEC; // This field specifies the desired address family for the returned addresses.
    hints.ai_socktype = SOCK_STREAM; // This field specifies the preferred socket type, for example SOCK_STREAM or SOCK_DGRAM.
    hints.ai_flags = AI_PASSIVE; // This field specifies additional options (AI_PASSIVE, AI_NUMERICHOST, AI_ADDRCONFIG, etc ...)
