...
class FrameWithLiDAR:
    def __init__(self, sequence, frame_id):
        ...
        rgb_file = os.path.join(self.rgb_dir, "{:010d}".format(frame_id) + ".png") #TODO 06 for odometry data, 010 for raw datasets
        self.velo_file = os.path.join(self.velo_dir, "{:010d}".format(frame_id) + ".bin") #TODO 06 for odometry data, 010 for raw datasets
        #print(rgb_file)
    ...
    def build_vizbox(self, bbox3d, rgb=[1, 0, 0]):
        ...
        colors[4] = [1 - rgb[0], 1 - rgb[1], 1 - rgb[2]]
    ...
    def get_detections(self):
        ...
        for n in range(detections_3d.shape[0]):
            ...
            trans, size, theta = det_3d[:3], det_3d[3:6], -det_3d[6]+np.pi/2   # FIXME mmdet3d 1.0.0rc6 -det_3d[6]+np.pi/2 , mmdet3d 0.18.1 det_3d[6]
            ...
            if show_3d:
                ...
                #w1,l1,h1 = list(size)
                l1,w1,h1 = list(size)
                line_set = self.transform_kitti_to_cuboid(w1, h1, l1, t, theta)
                line_set = self.build_vizbox(line_set, [0, 0, 1])
                #center = det_3d[0:3]
                #dim = det_3d[3:6]
                #yaw = np.zeros(3)
                #rot_axis = 2
...
class KITIISequence:
    def __init__(self, data_dir, configs):
        ...
        self.rgb_dir = os.path.join(data_dir, "2011_09_30_drive_0018_sync/image_02/data/") #TODO kitti_raw data
        self.velo_dir = os.path.join(data_dir, "2011_09_30_drive_0018_sync/velodyne_points/data/") #TODO kitti_raw data
        #self.rgb_dir = os.path.join(data_dir, "image_2/") #TODO dsp_slam
        #self.velo_dir = os.path.join(data_dir, "velodyne/") #TODO dsp_slam
    ...
    def get_labels_and_save(self):
        ...
        for frame_id in range(0, self.num_frames):
            ...
            torch.save(labels_2d, os.path.join(self.lbl2d_dir, "%010d.lbl" % frame_id)) #TODO 06 for odometry data, 010 for raw datasets
            torch.save(labels_3d, os.path.join(self.lbl3d_dir, "%010d.lbl" % frame_id)) #TODO 06 for odometry data, 010 for raw datasets
