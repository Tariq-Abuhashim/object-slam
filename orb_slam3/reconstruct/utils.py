...
#def load_velo_scan(file):
#    """Load and parse a velodyne binary file."""
#    scan = np.fromfile(file, dtype=np.dtype('i8, u4, f4, f4, f4, f4, u2') )#np.int64)
#    zeros = np.zeros_like(scan['f2'])
#    scan = np.vstack((scan['f4'], -scan['f3'], scan['f2'], scan['f6']/255)).ravel('F')
#    scan = scan.reshape((-1, 4))
#    #print(max(scan[:,3]))
#    return scan

# A custom type that behaves like a dictionary but throws an error when trying to access a key that doesn't exist.
# It inherits from Python's built-in Dict class and overwrites the __missing__ method. When you try to access a key 
#    that does not exist in this dictionary, instead of returning None like a normal dictionary, it will raise a KeyError.
