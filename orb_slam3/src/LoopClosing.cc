        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(bFindedRegion)
            {
                if(mbMergeDetected)
                {
                    ...
                    {
                        Sophus::SE3d mTmw = Converter::toSophus(mpMergeMatchedKF->GetPose()).cast<double>();
                        g2o::Sim3 gSmw2(mTmw.unit_quaternion(), mTmw.translation(), 1.0);
                        Sophus::SE3d mTcw = Converter::toSophus(mpCurrentKF->GetPose()).cast<double>();
                ...
                if(mbLoopDetected)
                {
                    ...
                    {
                        Sophus::SE3d Twc = Converter::toSophus(mpCurrentKF->GetPoseInverse()).cast<double>();
                        g2o::Sim3 g2oTwc(Twc.unit_quaternion(),Twc.translation(),1.0);
                        g2o::Sim3 g2oSww_new = g2oTwc*mg2oLoopScw;

                        Eigen::Vector3d phi = LogSO3(g2oSww_new.rotation().toRotationMatrix());
                        cout << "phi = " << phi.transpose() << endl; 
                        if (fabs(phi(0))<0.05f && fabs(phi(1))<0.05f && fabs(phi(2))<0.349f)
                        {
                            if(mpCurrentKF->GetMap()->IsInertial())
                            {
                                // If inertial, force only yaw
                    ...
                    if (bGoodLoop) {
                		...
                		//if(ComputeSim3()) // this used to be in ORB_SLAM2
						{
		                    if ( mpTracker->mSensor == System::STEREO || mpTracker->mSensor == System::IMU_STEREO //) // FIXME what other types of sensors
							|| (mpTracker->mSensor == System::IMU_MONOCULAR && WITH_LIDAR ) ) // FIXME what other types of sensors
							{
...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    if(mnLoopNumCoincidences > 0)
    {
        ...
        Sophus::SE3d mTcl = Converter::toSophus(mpCurrentKF->GetPose() * mpLoopLastCurrentKF->GetPoseInverse()).cast<double>();
    ...
    {
        // Find from the last KF candidates
        Sophus::SE3d mTcl = Converter::toSophus(mpCurrentKF->GetPose() * mpMergeLastCurrentKF->GetPoseInverse()).cast<double>();
                                                 ...
                                                 std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    if(nNumProjMatches >= nProjMatches)
    {
        ...
        Sophus::SE3d mTwm = Converter::toSophus(pMatchedKF->GetPoseInverse()).cast<double>();
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        if(numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            if(bConverge)
            {
                ...
                g2o::Sim3 gScm(Converter::toMatrix3d(solver.GetEstimatedRotation()), Converter::toVector3d(solver.GetEstimatedTranslation()), (double) solver.GetEstimatedScale());
                    ...
                    if(numOptMatches >= nSim3Inliers)
                    {
                        ...
                        if(numProjOptMatches >= nProjOptMatches)
                        {
                            ...
                            while(nNumKFs < 3 && j<vpCurrentCovKFs.size())
                            {
                                ...
                                Sophus::SE3d mTjc = Converter::toSophus(pKFj->GetPose() * mpCurrentKF->GetPoseInverse()).cast<double>();
                                ...
                                if(bValid)
                                {
                                    Sophus::SE3f Tc_w = Converter::toSophus(mpCurrentKF->GetPose());
                                    Sophus::SE3f Tw_cj = Converter::toSophus(pKFj->GetPoseInverse());
...
void LoopClosing::CorrectLoop()
{
    ...
    Sophus::SE3f Twc = Converter::toSophus(mpCurrentKF->GetPoseInverse());
    Sophus::SE3f Tcw = Converter::toSophus(mpCurrentKF->GetPose());
	...
	cv::Mat correctedTcw = Converter::toCvMat(mg2oLoopScw);  // FIXME what happens to scale here? this happens in many other places
...

    {
        ...
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            if(pKFi!=mpCurrentKF)
            {
                Sophus::SE3f Tiw = Converter::toSophus(pKFi->GetPose());
                ...
                //pKFi->SetPose(Converter::toSophus(correctedTiw).cast<float>());
				cv::Mat correctedTiw = Converter::toCvMat(g2oCorrectedSiw);  // FIXME what happens to scale here? this happens in many other places
    			mpCurrentKF->SetPose(correctedTcw);
        ...
        for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
        {
            ...
            for(size_t iMP=0, endMPi = vpMPsi.size(); iMP<endMPi; iMP++)
            {
                ...
                Eigen::Vector3d P3Dw = Converter::toVector3d(pMPi->GetWorldPos());
            ...
            if(bImuInit)
            {
                ...
                pKFi->SetVelocity(Converter::toCvMat(Rcor*Converter::toVector3f(pKFi->GetVelocity())));
            }
...
void LoopClosing::MergeLocal()
{
    ...
    for(KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        if(pKFi!=mpCurrentKF)
        {
            Sophus::SE3d Tiw = Converter::toSophus(pKFi->GetPose()).cast<double>();
		...
		g2o::Sim3 correctedTiw(g2oCorrectedSiw.rotation(), g2oCorrectedSiw.translation() / s, 1.0); // FIXME what happens to scale here? check many other places
		pKFi->mTcwMerge = Converter::toCvMat(correctedTiw);
        if(pCurrentMap->isImuInitialized())
        {
            Eigen::Quaternionf Rcor = (g2oCorrectedSiw.rotation().inverse() * vNonCorrectedSim3[pKFi].rotation()).cast<float>();
            pKFi->mVwbMerge = Converter::toCvMat(Rcor * Converter::toVector3f(pKFi->GetVelocity()));
        }
    ...
    while(itMP != spLocalWindowMPs.end())
    {
        ...
        pMPi->mPosMerge = Converter::toCvMat(eigCorrectedP3Dw);
        pMPi->mNormalVectorMerge = Converter::toCvMat(Rcor.cast<float>() * Converter::toVector3f(pMPi->GetNormal()));
    ...
    if(vpCurrentMapKFs.size() == 0){}
        ...
        if(mpTracker->mSensor == System::MONOCULAR)
        {
            ...
            for(KeyFrame* pKFi : vpCurrentMapKFs)
            {
                ...
                //Sophus::SE3d correctedTiw(g2oCorrectedSiw.rotation(),g2oCorrectedSiw.translation() / s);
				g2o::Sim3 correctedTiw(g2oCorrectedSiw.rotation(),g2oCorrectedSiw.translation() / s, 1.0); // FIXME what happens here to scale compared to other places

                pKFi->mTcwBefMerge = pKFi->GetPose();
                pKFi->mTwcBefMerge = pKFi->GetPoseInverse();

                pKFi->SetPose(Converter::toCvMat(correctedTiw));

                if(pCurrentMap->isImuInitialized())
                {
                    Eigen::Quaternionf Rcor = (g2oCorrectedSiw.rotation().inverse() * vNonCorrectedSim3[pKFi].rotation()).cast<float>();
                    pKFi->SetVelocity(Converter::toCvMat(Rcor*Converter::toVector3f(pKFi->GetVelocity()))); // TODO: should add here scale s
                }
...
void LoopClosing::MergeLocal2()
{
...

    {
        ...
        if(s_on!=1)
            bScaleVel=true;
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(Converter::toCvMat(T_on),s_on,bScaleVel);
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap, vector<MapPoint*> &vpMapPoints)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        int numFused = matcher.Fuse(pKFi,Converter::toCvMat(g2oScw),vpMapPoints,4,vpReplacePoints);
...
void LoopClosing::SearchAndFuse(const vector<KeyFrame*> &vConectedKFs, vector<MapPoint*> &vpMapPoints)
{
    ...
    for(auto mit=vConectedKFs.begin(), mend=vConectedKFs.end(); mit!=mend;mit++)
    {
        ...
        vector<MapPoint*> vpReplacePoints(vpMapPoints.size(),static_cast<MapPoint*>(NULL));
		matcher.Fuse(pKF,pKF->GetPose(),vpMapPoints,4,vpReplacePoints);
            ...
            while(!lpKFtoCheck.empty())
            {
                ...
                for(set<KeyFrame*>::const_iterator sit=sChilds.begin();sit!=sChilds.end();sit++)
                {
                    ...
                    if(pChild->mnBAGlobalForKF!=nLoopKF)
                    {
                        ...
                        Sophus::SO3f Rcor = Converter::toSophus(pChild->mTcwGBA).so3().inverse() * Converter::toSophus(pChild->GetPose()).so3();
                        if(pChild->isVelocitySet()){
                            pChild->mVwbGBA = Converter::toCvMat(Rcor * Converter::toVector3f(pChild->GetVelocity()));
                        }
                        else
