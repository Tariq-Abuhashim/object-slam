...
auto Communicator::ProcessKfBuffer()->void {
    ...
    while(!kf_out_buffer_.empty()) {
        ...
        if(kfi->sent_once_ && !covins_params::comm::send_updates) continue; // keyframe has been sent at least once before, and no need to send updates
        if(kfi->sent_once_ && kfi->mnId == 0) continue; // keyframe has been sent at least once before, and its the first keyframe
