...
public:
    ...
    #ifdef COVINS_MOD
	void static LocalJointBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, list<KeyFrame *> &local_kfs);
    #else
	void static LocalJointBundleAdjustment(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges);
    #endif
	#ifdef COVINS_MOD
	void static LocalJointInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, vector<KeyFrame *> &local_kfs, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit);
	#else
	void static LocalJointInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit);
	#endif
	void static FullInertialJointBA(Map *pMap, int its, const bool bFixLocal=false, const unsigned long nLoopKF=0, bool *pbStopFlag=NULL, bool bInit=false, float priorG = 1e2, float priorA=1e6, Eigen::VectorXd *vSingVal = NULL, bool *bHess=NULL);
